...
p_actual = st.sidebar.number_input(
    "Precio de Venta Actual",
    min_value=0.0,
    value=162000.00,
    format="%.2f"
)
q_actual = st.sidebar.number_input(
    "Unidades Vendidas Actuales",
    min_value=0,
    value=69000
)
mb_actual_pct = st.sidebar.number_input(
    "Margen Bruto Actual (%)",
    min_value=0.0,
    max_value=100.0,
    value=26.00,
...
with col_res:
    ...
    res1.metric(
        "Nuevo Margen Bruto",
        f"{nuevo_mb_pct:.2f}%",
        f"{nuevo_mb_pct - mb_actual_pct:.2f}%"
    )
    res2.metric(
        "Meta de Unidades",
        f"{int(q_necesaria):,}",
        f"{variacion_vol:.2f}% Vol."
    )

# --- ALERTA SI EL PRECIO NO DEJA UTILIDAD ---
if nuevo_p <= costo_unitario:
    st.error(
        "El nuevo precio es igual o inferior al costo unitario estimado. "
        "En este escenario no existe punto de indiferencia rentable, "
        "porque cada unidad genera utilidad bruta cero o negativa."
    )
...
if st.button("📄 Generar Reporte Gerencial de Punto de Indiferencia"):
    ...
    if nuevo_p <= costo_unitario:
        ...
        <div style="
            ...
            color: #1e1e1e;
        ">
            <h2 style="margin-top:0; color:#1e1e1e; text-align:center; font-size: 28px;">
                Reporte Gerencial de Punto de Indiferencia
            </h2>

            <p style="text-align:center; font-size:16px;">
                <strong>Desarrollado por Jaime Loaiza</strong>
            </p>

            <hr style="border:none; border-top:1px solid #ddd; margin: 25px 0;">

            <p style="font-size:19px; line-height:1.7; text-align:justify; margin-bottom: 18px;">
            ...
            <div style="
                ...
                line-height:1.6;
            ">
                <strong>⚠️ Consejo Gerencial:</strong><br><br>
        ...
        components.html(reporte_html, height=560, scrolling=True)
